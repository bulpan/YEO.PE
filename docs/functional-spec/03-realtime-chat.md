...
sequenceDiagram
    ...
    Client1->>Server: WebSocket 연결 (JWT 토큰)
    Server->>Server: 토큰 검증
    Server-->>Client1: connection 이벤트
    ...
    Server->>DB: 메시지 저장
    Server->>Server: 같은 방 멤버에게 브로드캐스트
...
sequenceDiagram
    ...
    Server->>Server: 같은 방의 다른 멤버에게만 전송
    Server-->>Client2: typing-indicator

    Note over Client1: 3초 후 자동으로 false
